...
### 5.2 Sin paso de "estado legal"/contrato que finaliza la habilitación de un curso (🔴 Alta — la brecha más grande del documento)
...
- **Go:** no existe ningún concepto de contrato/documento legal asociado a un `Provider` o `Course`. El flujo de `course_requests` (`backend/internal/course_requests/service.go:35-63`) va directo de `under_review` a `approved`/`rejected`, sin un estado intermedio "aprobado pero pendiente de contrato".
- **Impacto:** este es el hueco más grande del backend Go respecto al mock — sin este paso, no hay forma de modelar (ni de bloquear) la apertura de un curso que fue aprobado pedagógicamente pero cuyo proveedor aún no firmó el contrato correspondiente. Cualquier trabajo futuro de "cerrar la brecha" debería empezar por este punto, porque los gaps de §2.1 y §2.3 dependen de que este concepto exista primero.
...
| --- | ------------------------------------------------------------------------------------------------------------------------------ | ---------------- | --------- | ------ |
| 1   | Sin paso de "estado legal"/contrato que habilita un curso aprobado                                                             | §5.2             | 🔴 Alta    | Pendiente |
